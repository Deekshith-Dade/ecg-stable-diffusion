...
class VQVAEImg(VQVAEBase):
    ...
    def quantize(self, x):
        ...
        # Create one-hot encodings for each code usage
        encodings = torch.zeros(
            min_encoding_indices.shape[0] * min_encoding_indices.shape[1], self.codebook_size, device=x.device)
        encodings.scatter_(1, min_encoding_indices.view(-1, 1), 1)

        # Calculate average probability of each code
        avg_probs = torch.mean(encodings, dim=0)
        # Add small epsilon to avoid log(0)
        eps = 1e-10
        # Calculate perplexity: exp(-sum(p * log(p)))
        perplexity = torch.exp(-torch.sum(avg_probs *
                               torch.log(avg_probs + eps)))
        ...
        return QuantizeOutput(quant_out, perplexity, quantize_losses, encodings, min_encoding_indices)
    ...
    def encode(self, x) -> EncodeOutput:
        ...
        quant_output = self.quantize(out)
        return EncodeOutput(quant_output)

    def decode(self, z) -> DecodeOutput:
        out = z
