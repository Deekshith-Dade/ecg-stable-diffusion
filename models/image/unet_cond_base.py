...
class Unet(nn.Module):
    ...
    def __init__(self, im_channels, model_config):
        ...
        self.norm_out = nn.GroupNorm(
            self.norm_channels, self.conv_out_channels)
    ...
    def forward(self, x, t, cond_input=None):
        ...
        if self.image_cond:
            ...
            im_cond = torch.nn.functional.interpolate(
                im_cond, size=x.shape[-2:])
        ...
        if self.class_cond:
            ...
            class_embed = einsum(cond_input['class'].float(
            ), self.class_emb.weight, 'b n, n d -> b d')
            t_emb += class_embed
        ####################################
